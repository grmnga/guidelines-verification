...
module GenerateOpopTable
  ...
  def get_opop_links(links, level)
    ...
    "<p style='margin-bottom: 5px'><a href=#{links[:opop]}>ОПОП #{text_level}</a></p><p style='margin-bottom: 5px'><a href=#{links[:rpd]}>РПД</a></p>"
  end
  ...
  def get_edu_shed_link(links, year, level)
    year = '2020-2021' if (1...3).include? level
